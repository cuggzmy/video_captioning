...
def translate_batch(model, src_emb, opt):
    ...
    def beam_decode_step(inst_dec_beams, len_dec_seq, enc_output, inst_idx_to_position_map, n_bm, mode):
        ...
        def predict_word(dec_seq, dec_pos, enc_output, n_active_inst, n_bm):
            dec_output, dec_slf_attn_list, dec_enc_attn_list = model.module.decoder\
                (dec_seq, dec_pos, enc_output, return_attns=True)
            # print(dec_enc_attn_list[-1][0])
            dec_output = dec_output[:, -1, :]  # Pick the last step: (bh * bm) * d_h
            word_prob = F.log_softmax(model.module.cap_word_prj(dec_output), dim=1)
        ...
        word_prob = predict_word(dec_seq, dec_pos, enc_output, n_active_inst, n_bm)
    ...
    with torch.no_grad():
        ...
        src_enc = model.module.encoder(src_seq)
        ...
        # <---------------------------------------------Decode CAP ---------------------------------------------------->
        for len_dec_seq in range(1, 48+1):

            active_inst_idx_list = beam_decode_step(
                inst_dec_beams, len_dec_seq, src_enc, inst_idx_to_position_map, n_bm, mode='cap')
